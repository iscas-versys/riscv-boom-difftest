...
// DOC include start: BOOM Parameters
  ...
  /* for formal verification and difftest purposes */
  enableRiscvFormal : Boolean = true,
  enableDifftest    : Boolean = true,
  enablechirvFormal : Boolean = false
...
trait HasBoomCoreParameters extends freechips.rocketchip.tile.HasCoreParameters
{
  ...
  //***********************************
  // formal verification and difftest
  val RISCV_FORMAL = boomParams.enableRiscvFormal
  val DIFFTEST     = boomParams.enableDifftest
  val CHIRVFORMAL  = boomParams.enablechirvFormal
